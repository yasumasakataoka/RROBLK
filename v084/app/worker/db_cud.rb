      ...
      tbl_mktbls.each do |tbl_mktbl|
	      begin
	        ...
	        tbl[:message_code] = nil
			    strsql = "select * from #{tbl_mktbl[0]}  where id = #{tbl_mktbl[1]}  "
			    rec = ActiveRecord::Base.connection.select_one(strsql)
		      strsql = "select * from tblinks where codel like 'proc_tblink_mkbttable_#{rec["tblname"]}_%' and expiredate > current_date order by seqno"
		...
		if pare_search_key.size >=1  ###在庫引きあたり分は対象外
			...
			cmp_keys.each do |key|   ###
				if fullord_pare == "1"  ###下位の階層すべての部品すべて対象
					...
					### logic error next if nditms_id.empty?   ###同一itms_idで複数のopeitms_idがある時で子部品なし
					##str_itms = ""
					##nditms_id.each do |ndrec|
					##	str_itms << "  ( trn.opeitm_itm_id = #{ndrec["itms_id"].to_s}  and trn.opeitm_processseq = #{ndrec["processseq"].to_s}  )    or "
					##end
