(function(undef) {
    ...
    for_each(queue, function(q) {
        ...
        var item_list_string = item.join(',');
        var cv_price_string = cv_price.join(',');
        ...
        var queries = [
            ...
            'cv_price=' + encodeURIComponent(cv_price),
            'bc=1',
    ...
    function for_each(queue, callback, max_request) {
        var request_count = (status.request_count != undef) ? status.request_count : 0,
    ...
    if (window._fout_queue.tracker && !window._gaq) {
        append_script('//js.fout.jp/referrer.js');
    }

    if (window._fout_queue.redirect && window._fout_queue.redirect['is_redirect']) {
        append_script('//js.fout.jp/redirect.js');
    }
})();
