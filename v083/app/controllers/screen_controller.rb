...
class ScreenController < ApplicationController
  ...
  def nst  ###子画面　link   xxxS_IDでリンクする時と　ctlXXXでリンクする時で分かれる。
     ...
     pare_view = plsql.r_screens.first("where pobject_code_scr = '#{pare_code}' and screen_Expiredate > sysdate order by screen_expiredate ")[:pobject_code_view]  ## set @gridcolumns 
...
#####  親の項目をテーブルの上に表示
 ...
 #####
  ...
  def set_fields_from_allfields ###画面の内容をcommand_rへ
     ...
     command_r
     if show_data.empty? 
        render :text => "Create ScreenFields #{screen_code} by (crt_r_view_sql.rb  #{screen_code.split(/_/)[1]}) and restart rails "  and return
     end
...
##  end
 ...
 def  view_item_set   
   ...
   def set_aftershowform screen_code_view,show_data
	...
	##tmp = plsql.pobjects.all("where expiredate > sysdate and objecttype = '1' and code like '#{screen_code.split("_")[1].chop}%'  ")
	strtmp = ""
        keysfield = []
	##tmp.each do |i|
