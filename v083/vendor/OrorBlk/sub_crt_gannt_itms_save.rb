...
require "rubygems"
 ...
 def psub_get_contents(n0,r0)
     ...
     bgantt[n0[:seq].to_sym] = {:level=>n0[:level],:itm=>"#{itm[:code]}（ #{itm[:name]} ）",:loca=>"#{loca[:code]} （#{loca[:name]} ）",:duration=>(r0[:lt]||=0),:assigs=>"",:endtime=>n0[:endtime],:starttime=>n0[:endtime]-(r0[:lt]||=0)*24*60*60,:depends=>[]}
 ...
 def psub_get_next_process(n0,r0,endtime)
  ...
  if r then 
     ...
     cnt = 0
     @bgantts = {}
     @bgantts[:"000"] = {:level=>0,:itm=>"#{itm[:code]}（ #{itm[:name]} ）",:loca=>"",:duration=>"",:assigs=>"",:endtime=>Time.now,:starttime=>nil,:depends=>[]}
  ...
  ntime = (  @bgantts[:"000"][:endtime]  -  min_starttime )
  @bgantts[:"000"][:starttime] = min_starttime 
  @bgantts[:"000"][:duration] =   ((@bgantts[:"000"][:endtime]  - min_starttime).divmod(24*60*60))[0] 
  strgantt = %Q|{"#{ARGV[0]}":[|
  @bgantts.sort.each  do|key,value|
     strgantt << %Q|{"id":"#{value[:id]}","name":"#{value[:itm]}","code":"#{value[:loca]}","level":#{value[:level]},"start":#{(value[:starttime]+ntime).to_i.*1000},"duration":#{value[:duration]},"end":#{(value[:endtime]+ntime).to_i.*1000},"assigs":[],"depends":"#{value[:depends].join(':')}"},|
  end
   strgantt = strgantt.chop + %Q|],"selectedRow":0,"deletedTaskIds":[],"canWrite":true,"canWriteOnParent":true }|
  fprnt strgantt
  p "gantt  #{@bgantts}"
  time_now =  Time.now
  @bgantts[:"000"][:starttime] = time_now
  @bgantts[:"000"][:endtime] = time_now
  @bgantts[:"000"][:duration] =   0 
  strgantt = %Q|{"tasks":[|
  @bgantts.sort.each  do|key,value|
     strgantt << %Q|{"id":"#{value[:id]}","name":"#{value[:itm]}","code":"#{value[:loca]}","level":#{value[:level]},"start":#{(time_now).to_i.*1000},"duration":0,"end":#{(time_now).to_i.*1000},"assigs":[],"depends":""},|
  end
   strgantt = strgantt.chop + %Q|],"selectedRow":0,"deletedTaskIds":[],"canWrite":true,"canWriteOnParent":true }|
  fprnt strgantt
