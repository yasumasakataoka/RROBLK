...
### R_viewからh_tblを作成するapp
 def subprint str
    foo = File.open("crt_h_tbl.log","a")
    foo.puts str
    foo.close 
 end 
 def addmain i   ###  i=r_view
  if PLSQL::View.find(plsql, i.to_sym).nil?
       p	 "not exists view #{i}"
       exit(0)
    end
    ...
    selectstr = " select "
        ...
        sub_rtbl = "h_" + i.split("_",2)[1]   ##create tbl name
        subfields.each do |j|
	   js = j.to_s
           if  js =~ /_id/  then
